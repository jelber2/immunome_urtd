...
Command:
cd InDir = /work/jelber2/immunome_urtd/run1/clean-sort-addRG/
1.Uses samtools merge to combine stampy bam files
        ~/bin/samtools-1.1/samtools merge \
        /work/jelber2/immunome_urtd/combined/merged-bams/Sample.bam \
        /work/jelber2/immunome_urtd/run1/clean-sort-addRG/Sample-CL-RG.bam \
        /work/jelber2/immunome_urtd/run2/clean-sort-addRG/Sample-CL-RG.bam

2.Uses samtools flagstat to get alignment metrics on stampy aligned bam file
        cd /work/jelber2/immunome_urtd/combined/merged-bams/
...
(1)/work/jelber2/immunome_urtd/run1/clean-sort-addRG
(2)/work/jelber2/immunome_urtd/combined/merged-bams
(3)                                    /clean-sort-addRG-markdup
(4)                                    /realign-around-indels

InDir = /work/jelber2/immunome_urtd/combined/
Input Files = Sample-CL-RG.bam


Usage (execute following code in InDir):

~/scripts/immunome_urtd/05b-clean_sort_addRG_markdup_realign *-CL-RG.bam
...
if len(sys.argv)<2:
    ...
    for InFileName in FileList: # so samtools grabs only the file names (i.e., Samples)
        ...
        WallTime = "04:00:00"
        LogOut = "/work/jelber2/immunome_urtd/combined/clean-sort-addRG-markdup"
        LogMerge = "oe"
        JobName = "clean-sort-addRG-markdup-realign-%s" % (Sample)
        Command ="""
        ~/bin/samtools-1.1/samtools merge \
        /work/jelber2/immunome_urtd/combined/merged-bams/%s.bam \
        /work/jelber2/immunome_urtd/run1/clean-sort-addRG/%s-CL-RG.bam \
        /work/jelber2/immunome_urtd/run2/clean-sort-addRG/%s-CL-RG.bam
