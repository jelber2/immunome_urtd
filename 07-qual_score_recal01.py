...
07-qual_score_recal01.py - version 1.0

Command:
cd InDir = /work/jelber2/immunome_urtd/combined/realign-around-indels
...
File Info
InDir = /work/jelber2/immunome_urtd/combined/realign-around-indels
Input Files =
       Sample-realigned.bam
       ../call-SNPs-initial/ALL-samples-initial-Q30-SNPs.vcf
OutDir = /work/jelber2/immunome_urtd/combined/call-SNPs-recal01
Output Files = Sample-recal01.bam


Usage (execute following code in InDir):

find . -name '*-realigned.bam' -not -name 'ALL-samples-*' -exec ~/scripts/immunome_urtd/07-qual_score_recal01.py {} \;
...
if len(sys.argv)<2:
    ...
    for InFileName in FileList: # do the following steps for each file in the inputstream
        ...
        LogMerge = "oe"
        JobName = "qual_score_recal01-%s" % (Sample)
        Command ="""
        java -Xmx8g -jar ~/bin/GATK-3.3.0/GenomeAnalysisTK.jar \
        -T BaseRecalibrator \
