...
Command:
cd InDir = /work/jelber2/immunome_urtd/combined/call-SNPs-recal03
...
File Info
InDir = /work/jelber2/immunome_urtd/combined/call-SNPs-recal03
Input Files =
       Sample-recal03.bam
OutDir = /work/jelber2/immunome_urtd/combined/hc
Output Files = Sample-raw-snps-indels.vcf


Usage (execute following code in InDir):

find . -name '*-recal03.bam' -not -name 'ALL-samples-*' -exec ~/scripts/immunome_urtd/14-hc_per_sample.py {} \;
...
if len(sys.argv)<2:
    ...
    for InFileName in FileList: # do the following steps for each file in the inputstream
        ...
        WallTime = "01:00:00"
        LogOut = "/work/jelber2/immunome_urtd/combined/hc"
        LogMerge = "oe"
